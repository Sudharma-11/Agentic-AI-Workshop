## Installation

1. Clone the repository:
   ```
   git clone https://github.com/yourusername/appointment-booking-chatbot.git
   cd appointment-booking-chatbot
   ```

2. Install the required dependencies:
   ```
   pip install -r AgenticAIrequirements.txt
   ```
## Usage

1. Run the Streamlit application:
   ```
   streamlit run front.py
   ```

2. Open a web browser and navigate to the URL provided by Streamlit (usually `http://localhost:8501`)

3. Interact with the chatbot to book an appointment

## Project Structure

- `app.py`: Streamlit web application for the user interface which contains the agent declaration
- `tasks.py`: Initialization of tasks for the Agent.
-  `AgenticAIrequirements.txt`: List of Python dependencies
-  `agent.py`: Not integerated with the actual project but this script will help you understand on how to build an agent using langchain framework. 
